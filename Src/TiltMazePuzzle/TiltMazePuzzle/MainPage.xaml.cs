...
namespace TiltMazePuzzle
{
	...
	public partial class MainPage : ContentPage
	{
        ...
        public MainPage()
		{
            ...
            if (!AccFlag)
            {
                Xamarin.Forms.MessagingCenter.Subscribe<object, string>(this, "KeyDown", (sender, key) =>
                {
                    System.Diagnostics.Debug.WriteLine($"[DIAG] KeyDown via MessagingCenter: {key}");
                    switch (key)
                    {
                        case "Left":
        ...
        protected override void OnAppearing()
        {
            ...
            if (!AccFlag)
            {
                // Вместо предупреждения — подписка на клавиши
                this.Focus();
            }
            else
            {
...
{
    bool AccFlag = deviceInfo.GetInfo();
    System.Diagnostics.Debug.WriteLine($"[DIAG] OnKeyDown вызван, AccFlag={AccFlag}, Key={e.KeyCode}");
    if (!AccFlag)
    {
        string diagMsg = $"[DIAG] KeyDown: {e.KeyCode}";
        ...
        System.Diagnostics.Debug.WriteLine($"[DIAG] acceleration={acceleration}");
        return true;
    }
